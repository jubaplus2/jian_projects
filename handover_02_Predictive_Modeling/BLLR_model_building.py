...
def run_predicting():
    ...
    nth_cutoff_X_Y=4 # the column index

    # In[3]:

# check id count in each table
    ...
    def generate_gain_chart_function(df_X,list_y,list_ids,result_sm_model,threshold,list_selected_features):
        ...
        return df_gainchart
    output_folder=model_output_folder+"output_No_DCM_%s_%s/"%(str(df_date_range.iloc[0,1]),str(datetime.datetime.now().date()))
    try:
    ...
    class SM_Logistic_Model_dvN:
        # 1
        def __init__(self,n_week_DV,key_df_type,df_date_range,sql_engine=BL_engine,dict_cols_in_table=dict_cols_in_table,output_folder=output_folder):
    ...
    # sql_engine: mysql engine to BigLots database
        ...
        def select_test_df_from_mysql(self):
            ...
            cols_Y=[x for x in self.df_test_X.columns.tolist() if "cumulative" in x]
            self.df_test_Y=self.df_test_X[cols_Y]
            for col in cols_Y:
                del self.df_test_X[col]
        ...
        def generate_step_table_of_test_SM(self,):
            ...
            if self.key_df_type=="trans_2_plus":
                ...
                margin=max(np.round(self.pctg/2,2),0.03)
    ...
    # Option for memory flow: overwrite the training df for 1 time buyer in the python
    df_train_trans_1_only_X=pd.DataFrame()
    df_train_trans_1_only_Y=pd.DataFrame()
    list_train_trans_1_only_id=[]
    gc.collect()
