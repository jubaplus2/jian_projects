...
for file in list_remote_impr_files:
    ...
    if ("_impression_" in basename) & (basename.split("_impression_")[1][:8]>=min_date_needed):
...
for file in list_remote_click_files:
    ...
    if ("_click_" in basename) & (basename.split("_click_")[1][:8]>=min_date_needed):
...
for file in list_remote_act_files:
    basename=os.path.basename(file)
    if ("_activity_" in basename) & (basename.split("_activity_")[1][:8]>=min_date_needed):
