...
path = '/disk2/Projects/Rentrak/20160208-20160221/'
...
def ncreatoutput1():
	...
	lheader, weeklength, prevweekdf, weekinfodict = previousweekFile('Show2437680W10.csv')
	print lheader
	print 'hit ncreatoutput1'
	print ' first previousweekFile written'
	header = '\t'.join(lheader)+'\n'
	f = open('Show2437680W11.csv','w')
        f.write(header)
        f.close()
	...
	showidToName = {'2437680': ('WETV', 'Growing Up Hip Hop')}
				...
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
							...
							df.to_csv('Show2437680W11.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	prevweekdf.to_csv('Show2437680W11.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
