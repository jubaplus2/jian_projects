...
from anaconda_navigator.config import (CHANNELS_PATH, CONF, DEFAULT_BRAND, MAC,
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.stack.add_social(
            'Youtube',
            url="https://www.youtube.com/c/continuumio",  # FIXME:
        )
        self.stack.add_social(
            'Github',
            url="https://github.com/ContinuumIO",  # FIXME:
        )
    ...
    def setup(self, worker, output, error):
        ...
        if self.initial_setup:
            ...
            self.check_for_updates(packages=packages)
    ...
    def check_for_updates(self, packages=None, version=None):
        ...
        if packages:
            ...
            if package_data:
                ...
                if versions and (
                    lv(versions[-1]) > lv(navi_version) or
                    'dev' in navi_version and
                    versions[-1] == navi_version.replace('dev', '')
                ):
                    ...
                    if not self.config.get('main', 'hide_update_dialog'):
                        self.update_application(center_dialog=True)
    ...
    def update_application(self, center_dialog=False):
        ...
        if version:
            dlg = DialogUpdateApplication(
                version=version, startup=center_dialog, qa_testing=qa_testing
            )
            ...
            if self._dialog_update is None:
                ...
                if dlg.exec_():
                    ...
                    self.open_updater(version)
                    self.close()

            self._dialog_update = None
            self._track_tab()

    def open_updater(self, version):
    ...
    def show_create_environment(self):
        ...
        dlg = CreateDialog(parent=self)
        self._dialog_environment_action = dlg
        worker_info = self.api.conda_info()
...
#                self.stop_refreshing_timer()
    ...
    def show_conda_packages_action(
        ...
        pkgs = install_packages if install_packages else remove_packages
        self.set_busy_status(conda=True)
        if install_packages:
    ...
    def show_cancel_process(self):
        ...
        if self.is_busy():
            ...
            self._dialog_quit_busy
            if dlg.exec_():
                self.update_status(action='Process cancelled')
                self.api.conda_terminate()
    ...
    def launch_application(
        ...
        logger.debug(
            ...
            'prefix: {3}, sender:{4}'.
            format(package_name, command, leave_path_alone, prefix, sender)
        )
    ...
    def check_dependencies_before_install(self, worker, output, error):
        ...
        if conflicts:
            ...
            self._dialog_environment_action = dlg
            worker_info = self.api.conda_info(prefix=self.current_prefix)
            worker_info.sig_chain_finished.connect(dlg.setup)
...
#        self.check_dependencies_before_install(worker, worker.output, error)
    ...
    def conda_application_action(self, action, package_name, version, sender):
        """FIXME:."""
        logger.debug(
        ...
        if action == C.APPLICATION_INSTALL:
            ...
            action_msg = (
                '{0} application <b>{1}</b> on '
                '<b>{2}</b>'
            ).format(text_action, package_name, self.current_prefix)
