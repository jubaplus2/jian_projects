...
wrtier="/home/jian/Projects/Saatva/GA/gmap_3.csv"
for i in range(len(gmap_3)):
    lat = gmap_3.iloc[i,0]
    long = gmap_3.iloc[i,1]
    reverse_geocode_result = gmaps_3_key.reverse_geocode((lat, long))
    ...
    for j in range(len(address_components_list)):
        if address_components_list[j]['types']==['country', 'political']:
            gmap_3['Country'][i]=address_components_list[j]['short_name']
        if address_components_list[j]['types']==['postal_code']:
            gmap_3['zip_code'][i]=address_components_list[j]['short_name']
            
gmap_3.to_csv(wrtier,index=False)
