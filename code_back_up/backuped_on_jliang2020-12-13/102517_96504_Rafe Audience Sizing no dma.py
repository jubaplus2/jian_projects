...
df['finalsegment']=np.where((df['A1']>=3)&(df['MVPD']=='Dish Network'),'A1',
                            np.where((df['A1']>=2)&(df['MVPD']=='Dish Network'),'A2',
                            np.where((df['A1']>=3)&(df['MVPD']=='Dish Network'),'A1',
                            np.where((df['A1']>=2)&(df['MVPD']=='Dish Network'),'A2',
                            'Other'))))

df.to_csv("AudienceSizing_rafe no dma.csv",index=False)
