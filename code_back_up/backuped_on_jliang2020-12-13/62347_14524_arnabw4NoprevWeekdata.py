...
def ncreatoutput1():
	...
	#header = '\t'.join(lheader)+'\n'
	header = 'ShowID\tNetwork_no\tShowName\tUSNG_100\tZip\tZip4\tRank20150907-20150920\tRank20150921-20151004\tRank20151005-20151018\tRank20151019-20151101\tRank20151102-20151115\tRank20151116-20151129\tRank20151130-20151213\tRank20151214-20151227\tRank20151228-20160110\tRank20160111-20160124\tRank20160125-20160207\t\n'
	print header
	#w0,w1,w2,w3,w4...w10 so 11 length
	weeklength = 11 
	f = open('Show2437680W10.csv','w')
        f.write(header)
        f.close()
			...
			for elements1 in showIDToUsngAndRank:
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						df.to_csv('Show2437680W10.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
