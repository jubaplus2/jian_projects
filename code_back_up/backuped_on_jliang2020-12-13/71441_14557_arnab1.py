...
def ncreatoutput1():
	...
	lheader, weeklength, prevweekdf, weekinfodict = previousweekFile('tt2.csv')
	print lheader
					...
					for elements in usngzip4list:
						...
						if elements1[0] == '16TCM990011':
							...
							if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
								...
								df.to_csv('optoutput12.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	gc.collect()
	prevweekdf.to_csv('optoutput12.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
