...
excludeworldlist = list(set(['https', 'pricing', 'plans', 'sign', 'join', 'sem', 'forum', 'help','video', 'phrase', 'sort', 'true', 'false', 'license', 'creative'])| additionallist())
...
wordstoberemoved = cachedStopWords | set(['http', 'www', 'istockphoto', 'com', 'php', 'gettyimages'])
...
#segementList = ['photo','creative', 'editorial', 'video', 'music']
...
def getWordsFromURL(input):
	...
	#if not set(lower) & set(excludeworldlist):
	vv = list(set(lower) - wordstoberemoved - set(excludeworldlist))
...
def geturltoworddict(vv):
	urltoworddict = {}
	for x in vv:
		vv1 = getWordsFromURL(x)
		if vv1:
			wordlist = validWord(vv1)
...
if __name__ == "__main__":
	v = pd.read_csv('/home/account1/Gettysegmentation/iStockUSURL.txt', '\t')
	#v = pd.read_csv('nlptest.csv', '\t')

	#tt = removenoiseurl(v)
	tt = list(set(v['URL']))
	urltoworddict = geturltoworddict(tt)
	#with open('/disk2/Projects/Gettyurlanalysis/wordremoved/GettyUSURLOutput.txt','w') as f:
	with open('iStockUSURLOutput.txt','w') as f:
