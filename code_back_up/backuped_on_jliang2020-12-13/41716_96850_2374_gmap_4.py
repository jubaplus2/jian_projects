...
wrtier="/home/jian/Projects/Saatva/GA/gmap_4.csv"
for i in range(len(gmap_4)):
    lat = gmap_4.iloc[i,0]
    long = gmap_4.iloc[i,1]
    reverse_geocode_result = gmaps_4_key.reverse_geocode((lat, long))
    ...
    for j in range(len(address_components_list)):
        if address_components_list[j]['types']==['country', 'political']:
            gmap_4['Country'][i]=address_components_list[j]['short_name']
        if address_components_list[j]['types']==['postal_code']:
            gmap_4['zip_code'][i]=address_components_list[j]['short_name']
            
gmap_4.to_csv(wrtier,index=False)
