...
logging.basicConfig(filename='/home/jian/Projects/Big_Lots/Analysis/2019_Q4/Predictive_Model_Building/save_dcm_lr_to_mysql/Activity_Table_Initiation_log.log', level=logging.INFO)
...
def create_Pred_act_table_BL_id():
    with engine_pymysql_db_BL.cursor() as cur:
        ...
        cur.execute("CREATE TABLE Pred_Activity_BL_id(Event_Time bigint, date_utc date, time_utc time, User_ID varchar(64), Customer_Link varchar(128), customer_id_hashed varchar(64), Activity_ID varchar(16), url varchar(2048), search_term varchar(1024), session_sequence int, activity_sequence int);")
...
def create_Pred_act_table_GU_id():
    with engine_pymysql_db_BL.cursor() as cur:
        ...
        cur.execute("CREATE TABLE Pred_Activity_GU_id(Event_Time bigint, date_utc date, time_utc time, User_ID varchar(64), Customer_Link varchar(128), customer_id_hashed varchar(64), Activity_ID varchar(16), url varchar(2048), search_term varchar(1024), session_sequence int, activity_sequence int);")
...
def get_url_and_kwd(url):
    ...
    if '?ntt' in url:
        return term_detail.replace('+',' ').replace('=',' ').replace('-',' ').strip()
...
for month, df_file_month in df_log_files_by_date.groupby("month"):
    ...
    df_month_act_GUID.to_sql("Pred_Activity_GU_id",if_exists='append', con=BL_engine, index=False,chunksize=300000,
                    dtype={
                        ...
                        'search_term':sqlalchemy.types.VARCHAR(length=1024),
