...
wrtier="/home/jian/Projects/Saatva/GA/gmap_7.csv"
for i in range(len(gmap_7)):
    lat = gmap_7.iloc[i,0]
    long = gmap_7.iloc[i,1]
    reverse_geocode_result = gmaps_7_key.reverse_geocode((lat, long))
    ...
    for j in range(len(address_components_list)):
        if address_components_list[j]['types']==['country', 'political']:
            gmap_7['Country'][i]=address_components_list[j]['short_name']
        if address_components_list[j]['types']==['postal_code']:
            gmap_7['zip_code'][i]=address_components_list[j]['short_name']
            
gmap_7.to_csv(wrtier,index=False)
