...
logging.basicConfig(filename='/home/jian/Projects/Big_Lots/Analysis/2019_Q4/Predictive_Model_Building/save_dcm_lr_to_mysql/ExposureV2_Table_Initiation_log.log', level=logging.INFO)
...
def create_Pred_Exposure_table_BL_id():
    with engine_pymysql_db_BL.cursor() as cur:
        cur.execute("DROP TABLE IF EXISTS Pred_ExposureV2_BL_id")
        cur.execute("CREATE TABLE Pred_ExposureV2_BL_id(User_ID varchar(64), Customer_Link varchar(128), customer_id_hashed varchar(64), date_est date, Advertiser_ID varchar(16), Campaign_ID varchar(16), Ad_ID varchar(16), Site_ID varchar(16), Placement_ID varchar(16), impressions int, clicks int,  week_end_dt date );")
    print("An empty TABLE Pred_ExposureV2_BL_id has been created.",datetime.datetime.now())
    logging.info("An empty TABLE Pred_ExposureV2_BL_id has been created: "+str(datetime.datetime.now()))
...
for date_week,df_group_log_both_week in df_both_log_files_by_date.groupby(["date","week_end_dt"]):
    ...
    if df_group_log_both_week['type'].tolist()!=['click','impression']:
        ...
        if filedate_dateformat==week_end_1_week_ago+datetime.timedelta(days=7):
            ...
            df_BL_to_sql.to_sql("Pred_ExposureV2_BL_id",if_exists='append', con=BL_engine, index=False,chunksize=300000,
                    dtype={
