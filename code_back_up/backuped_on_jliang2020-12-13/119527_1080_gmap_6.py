...
wrtier="/home/jian/Projects/Saatva/GA/gmap_6.csv"
for i in range(len(gmap_6)):
    lat = gmap_6.iloc[i,0]
    long = gmap_6.iloc[i,1]
    reverse_geocode_result = gmaps_6_key.reverse_geocode((lat, long))
    ...
    for j in range(len(address_components_list)):
        if address_components_list[j]['types']==['country', 'political']:
            gmap_6['Country'][i]=address_components_list[j]['short_name']
        if address_components_list[j]['types']==['postal_code']:
            gmap_6['zip_code'][i]=address_components_list[j]['short_name']
            
gmap_6.to_csv(wrtier,index=False)
