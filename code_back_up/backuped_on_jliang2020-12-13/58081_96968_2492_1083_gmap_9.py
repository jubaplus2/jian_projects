...
wrtier="/home/jian/Projects/Saatva/GA/gmap_9.csv"
for i in range(len(gmap_9)):
    lat = gmap_9.iloc[i,0]
    long = gmap_9.iloc[i,1]
    reverse_geocode_result = gmaps_9_key.reverse_geocode((lat, long))
    ...
    for j in range(len(address_components_list)):
        if address_components_list[j]['types']==['country', 'political']:
            gmap_9['Country'][i]=address_components_list[j]['short_name']
        if address_components_list[j]['types']==['postal_code']:
            gmap_9['zip_code'][i]=address_components_list[j]['short_name']
            
gmap_9.to_csv(wrtier,index=False)
