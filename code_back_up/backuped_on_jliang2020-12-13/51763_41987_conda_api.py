    ...
    def communicate(self):
        ...
        if 'using anaconda' not in stderr.lower():
            if stderr.strip() and self._conda:
                ...
                logger.warning('Conda command output on stderr {0}'.format(d))
        ...
        if self._parse and stdout:
            ...
            if 'exception_name' in result[0] or 'exception_type' in result[0]:
                ...
                error = '{0}: {1}'.format(
                    " ".join(self._cmd_list), result[0]['message']
                )
...
class _CondaAPI(QObject):
    ...
    def set_conda_prefix(self, conda_prefix=None, root_prefix=None):
        """
        ...
        if not conda_prefix and not root_prefix:
    ...
    def config_show(
        ...
        if sources:
            ...
            worker = self._call_and_parse(
                cmd_list,
                abspath=abspath,
                callback=lambda o, e: self._config_show_sources(
                    o, e, prefix=prefix, all_=all_
                ),
                environ=environ,
            )
        else:
