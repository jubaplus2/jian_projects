...
df['finalsegment']=np.where((df['ga1']==1)&(df['MVPD'].isin(['DirecTV','Dish Network']))&(df['LPMCode'].isin(['101', '104','105','106','110','111','112','117','124','139','202','218'])),'A1',
                            ...
                            np.where((df['ot1']>0)&(df['LPMCode'].isin(['101', '104','105','106','110','111','112','117','124','139','202','218']) & (df['Age'] >= 40) & (df['Age'] <=54)),'B1',np.where((df['ot2']>0)&(df['LPMCode'].isin(['101', '104','105','106','110','111','112','117','124','139','202','218']) & (df['Age'] >= 40) & (df['Age'] <=54)),'B2','Other')))))))

print(df[['finalsegment','HHID_PersonID']].groupby('finalsegment').count())
df.to_csv("AudienceSizing_AS_241_DMA_new2.csv",index=False)
print(datetime.datetime.now())
