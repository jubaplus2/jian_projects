...
def ncreatoutput1():
	...
	lheader, weeklength, prevweekdf, weekinfodict = previousweekFile('Show1929314W10.csv')
	print lheader
	print 'hit ncreatoutput1'
	print ' first previousweekFile written'
	header = '\t'.join(lheader)+'\n'
	f = open('Show1929314W11.csv','w')
        f.write(header)
        f.close()
				...
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
							...
							df.to_csv('Show1929314W11.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	prevweekdf.to_csv('Show1929314W11.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
