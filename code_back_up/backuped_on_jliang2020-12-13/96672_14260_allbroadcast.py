	...
	for v in fin:
		if v[0:2] == '36':
			...
			if int(personid) != 0 and programid == programidinfo:
				...
				key=programid+telecastid+':'+viewershipdate+hhid+personid;
...
def populateAllTable30(inputfile):
        ...
        for v in fin:           
			...
			if programid == programidinfo:
				telecastid = v[21:28];
				key = programid+telecastid;
				if entry.get(key) is None:
...
def merging(fout,entry36,entry30,hhidweightsdict, hhidpersonidweightsdict):
        for key in entry36:
                ...
                result30 = '\t'.join([lhhidalldetailsTable30.programid, lhhidalldetailsTable30.programid,lhhidalldetailsTable30.telecastid,
...
def generatingoutput(fout,inputfile2, inputfile3, inputfile4):
