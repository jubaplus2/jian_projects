				...
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
							...
							df.to_csv('optoutput2375987.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	gc.collect()
	prevweekdf.to_csv('optoutput2375987.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
