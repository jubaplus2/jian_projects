...
def ncreatoutput1():
	...
	gc.collect()
	lheader, weeklength, prevweekdf, weekinfodict = previousweekFile('/disk2/arnab/rentrak/week1/rentrakoutputW1.csv')
	print lheader
				...
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
							...
							df.to_csv('Show2294631W2.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	gc.collect()
	prevweekdf.to_csv('Show2294631W2.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
