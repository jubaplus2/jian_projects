...
df['LPMCode'] = np.where(df['LPMCode']=='   ','999',df['LPMCode'])
df['LPMCode'] = df['LPMCode'].astype('int')
df['finalsegment']=np.where((df['A1']>=3)&(df['LPMCode'].isin(top10dma)),'A1',
                            np.where((df['A1']>=2)&(df['LPMCode'].isin(top10dma)),'A2',
                            np.where((df['A1']>=3)&(df['LPMCode'].isin(non10dma)),'A3',
                            np.where((df['A1']>=2)&(df['LPMCode'].isin(non10dma)),'A4',
                            'Other'))))

df.to_csv("AudienceSizing_rafenodish.csv",index=False)
