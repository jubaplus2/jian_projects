...
"""
import logging
logging.basicConfig(filename='WGNA_l3m.log', level=logging.INFO)
...
##### update Cable ID (WeTV: '007516',SUND: '007602',STARZ: '009157', WGNA: '006788', aen: '006218',LIF: '006196')
Cable_ID='006788'

##### update the outputs path and location
finalfiledic='/home/nielsen/MicroSegment/Outputs/Quads/WGNA_l3m_180318.csv'
...
allfile=allfile[['HHID', 'CableID', 'ViewDate', 'PersonID',
       ...
       'ProgramName', 'StartTime', 'EndTime','PersonIntab',
        #Jay
        'Sex Code', 'Age']]
...
totalfreq=allfile[['HHID', 'PersonID']]
totalfreq['freq'] = 1
totalfreq=totalfreq.groupby(['HHID', 'PersonID']).sum()
totalfreq.reset_index(inplace=True)  
totalfreq.columns=['HHID', 'PersonID', 'Totalfreq']

totalmins = pd.merge(totalmins,totalfreq,on=['HHID', 'PersonID'])
totalmins['Quads']=1
totalmins = totalmins[['HHID', 'PersonID','Totalfreq','Totalmins','Quads']]

totalmins=totalmins.sort_values('Totalfreq',ascending=0)
totalmins=totalmins.reset_index(drop=True)
totalmins=totalmins.reset_index()
totalmins.columns=['freqrank','HHID', 'PersonID','Totalfreq', 'Totalmins','Quads']
totalmins=totalmins.sort_values('Totalmins',ascending=0)
totalmins=totalmins.reset_index(drop=True)
totalmins=totalmins.reset_index()
totalmins.columns=['minsrank','freqrank','HHID', 'PersonID','Totalfreq', 'Totalmins','Quads']
rows=len(totalmins.index)
totalmins['Cluster']=np.where(((totalmins['freqrank']<=rows/10)|(totalmins['minsrank']<=rows/10)),'Core_Occ',np.where(((totalmins['freqrank']<=rows/(10/3))|(totalmins['minsrank']<=rows/(10/3))),'Core_Occ','Balance'))

totalmins['HHID_PersonID'] = totalmins['HHID'] + '_' + totalmins['PersonID']

allfile['HHID_PersonID'] = allfile['HHID'] + '_' + allfile['PersonID']
personintab = allfile[['HHID_PersonID','ViewDate', 'PersonIntab']].drop_duplicates()
personintab = personintab.groupby('HHID_PersonID').mean()
personintab.reset_index(inplace = True)

dfdemo = allfile[['HHID_PersonID','Sex Code', 'Age']].drop_duplicates('HHID_PersonID')
totalmins = pd.merge(totalmins,personintab,on = 'HHID_PersonID',how = 'left')
totalmins = pd.merge(totalmins,dfdemo,on = 'HHID_PersonID',how = 'left')
