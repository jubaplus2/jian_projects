...
wrtier="/home/jian/Projects/Saatva/GA/gmap_8.csv"
for i in range(len(gmap_8)):
    lat = gmap_8.iloc[i,0]
    long = gmap_8.iloc[i,1]
    reverse_geocode_result = gmaps_8_key.reverse_geocode((lat, long))
    ...
    for j in range(len(address_components_list)):
        if address_components_list[j]['types']==['country', 'political']:
            gmap_8['Country'][i]=address_components_list[j]['short_name']
        if address_components_list[j]['types']==['postal_code']:
            gmap_8['zip_code'][i]=address_components_list[j]['short_name']
            
gmap_8.to_csv(wrtier,index=False)
