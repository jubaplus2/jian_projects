...
if __name__ == "__main__":
	...
	for key in printHeader:	
		...
		lvalues = copy.deepcopy(values)
                lheader = header
		if 'Name' in header:
			#Note name is under col 2
                        del lvalues[2]
                        lheader = lheader.replace('Name\t', '')
		payload= generateconfigurationAutomationStep4(lvalues, includeDict, excludeDict, locationDict,permDict, conditionDict,  lheader)
		if json.dumps(payload['targeting_spec']) not in lsofarList:
                                print 'adsetCreation'
                                payload['targeting'] = payload['targeting_spec']
                                t = [values[0], '_'.join(values[1]), '_'.join(values[2:])]
                                #payload['name'] = key
                                adname = '_'.join(t)
                                payload['name'] = adname
				if 'Name' in header:
                                	payload['name'] = values[2]
					adname = values[2]
                                payload['billing_event'] = 'IMPRESSIONS'
                                payload['access_token'] = config['access_token']
                                if lvalues[4] == 'Lifetime Budget':
                                        payload['is_autobid'] = 'TRUE'
                                        #remember the format is $100.00
                                        payload['lifetime_budget'] = lvalues[5].strip().split('$')[-1].replace('.','')
                                        payload['status']= 'PAUSED'
                                        payload['start_time'] =  lvalues[6]+ ' '+'EST'
                                        payload['end_time'] =  lvalues[7]+ ' '+'EST'
                                del  payload['targeting_spec']
				if payload['optimization_goal'] == 'STORE_VISITS':
					payload['promoted_object'] = json.dumps({"place_page_set_id":"2021969388019090"})
					#payload['promoted_object'] = json.dumps({"place_page_set_id":lvalues[1]})
