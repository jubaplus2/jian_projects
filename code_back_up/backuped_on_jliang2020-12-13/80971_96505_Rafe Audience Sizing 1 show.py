...
df['finalsegment']=np.where((df['A1']>=3)&(df['LPMCode'].isin(top10dma))&(df['MVPD']=='Dish Network'),'A1',
                            np.where((df['A1']>=2)&(df['LPMCode'].isin(top10dma))&(df['MVPD']=='Dish Network'),'A2',
                            np.where((df['A1']>=3)&(df['LPMCode'].isin(non10dma))&(df['MVPD']=='Dish Network'),'A3',
                            np.where((df['A1']>=2)&(df['LPMCode'].isin(non10dma))&(df['MVPD']=='Dish Network'),'A4',
                            'Other'))))

df.to_csv("AudienceSizing_rafe 1 show.csv",index=False)
