...
df['finalsegment']=np.where((df['Quads']>0)&(df['A1']>0),'A1',
                            ...
                            np.where((df['A3']>0)&((df['Race']==1)&(df['Age']>=35)&(df['Age']<=54)&(df['Gender']=='F')),'A3',
                            np.where((df['A4']>0)&((df['Race']==1)&(df['Age']>=35)&(df['Age']<=54)&(df['Gender']=='F')),'A4',
                            np.where(df['OT']>0,'OT','Other')))))

df.to_csv("AudienceSizing_faA.csv",index=False)
