...
def ncreatoutput1():
	...
	#header = '\t'.join(lheader)+'\n'
	header = 'ShowID\tNetwork_no\tShowName\tUSNG_100\tZip\tZip4\tRank20150907-20150920\tRank20150921-20151004\tRank20151005-20151018\tRank20151019-20151101\tRank20151102-20151115\tRank20151116-20151129\tRank20151130-20151213\n'
	print header
	#w0,w1,w2,w3,w4,w5,w6 so 5 length
	weeklength = 7 
	f = open('Show2172792W6.csv','w')
        f.write(header)
        f.close()
			...
			for elements1 in showIDToUsngAndRank:
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						df.to_csv('Show2172792W6.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
