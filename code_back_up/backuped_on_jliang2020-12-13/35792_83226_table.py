...
class TableCondaPackages(QTableView):
    ...
    def __init__(self, parent):
        ...
        self._hheader.setStyleSheet(
            """QHeaderView {border: 0px; border-radius: 0px;};"""
        )
    ...
    def setup_model(self, packages, data, metadata_links=None):
        ...
        def filter_text(row, text, status):
            ...
            in_name = all(
                t in row[C.COL_NAME].lower()
                for t in to_text_string(text).lower().split()
            )
            in_desc = all(
                t in row[C.COL_DESCRIPTION].lower()
                for t in to_text_string(text).split()
            )
    ...
    def filter_changed(self):
        ...
        if group in [C.ALL]:
            group = '-'.join(
                [
                    to_text_string(C.INSTALLED), to_text_string(C.UPGRADABLE),
            ...
            group = '-'.join(
                [to_text_string(C.UPGRADABLE), to_text_string(C.MIXGRADABLE)]
            )
    ...
    def resizeEvent(self, event):
        ...
        w_new = w - (
            width_start + self.WIDTH_ACTIONS + self.WIDTH_TYPE +
    ...
    def mousePressEvent(self, event):
        ...
        elif (column == C.COL_VERSION and model.is_upgradable(model_index) and
                left_click):
            # 1-click update
    ...
    def set_action_status(
        self, model_index, status=C.ACTION_NONE, version=None
    ):
    ...
    def context_menu_requested(self, event, right_click=False):
        ...
        if column in [C.COL_ACTION] and left_click:
            ...
            action_install = create_action(
                ...
                toggled=lambda: self.
                set_action_status(model_index, C.ACTION_INSTALL)
            )
            ...
            for version in reversed(versions):
                ...
                if version == current_version:
                    version_action = create_action(
                        ...
                        icon=QIcon(),
                        triggered=trigger(
                            model_index, C.ACTION_INSTALL, version
                        )
                    )
                    ...
                    version_action = create_action(
                        ...
                        icon=QIcon(),
                        triggered=trigger(
                            model_index, upgrade_or_downgrade_action, version
                        )
                    )
