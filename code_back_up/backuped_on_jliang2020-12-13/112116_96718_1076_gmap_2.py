...
wrtier="/home/jian/Projects/Saatva/GA/gmap_2.csv"
for i in range(len(gmap_2)):
    lat = gmap_2.iloc[i,0]
    long = gmap_2.iloc[i,1]
    reverse_geocode_result = gmaps_2_key.reverse_geocode((lat, long))
    ...
    for j in range(len(address_components_list)):
        if address_components_list[j]['types']==['country', 'political']:
            gmap_2['Country'][i]=address_components_list[j]['short_name']
        if address_components_list[j]['types']==['postal_code']:
            gmap_2['zip_code'][i]=address_components_list[j]['short_name']
            
gmap_2.to_csv(wrtier,index=False)
