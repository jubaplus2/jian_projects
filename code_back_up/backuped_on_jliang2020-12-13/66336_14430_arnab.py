...
def ncreatoutput1():
	...
	lheader, weeklength, prevweekdf, weekinfodict = previousweekFile('Show2089348.csv')
	#lheader, weeklength, prevweekdf, weekinfodict = previousweekFile('test.csv')
	print lheader
				...
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
							...
							df.to_csv('optoutput3.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	prevweekdf.to_csv('optoutput3.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
