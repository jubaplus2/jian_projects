...
def previousweekFile(inputfile):
	...
	v.loc[:,newcol] = '0'
        t = list(v.keys())
        t.append(newcol)
	weekinfodict = v.iloc[0:,0:6]
        return t, len(t[6:]),v, weekinfodict

def searchinDF(prevweekdf,weekinfodict, inputlist, newrank):
	tt1 = weekinfodict.isin(inputlist).all(1)
	try:
		index = tt1[tt1 == True].index[0]
	except:
		index = -1
		pass
	if index >=0:
		prevweekdf.iloc[index, -1] =  newrank
				...
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
							...
							df.to_csv('optoutput2.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	gc.collect()
	prevweekdf.to_csv('optoutput2.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
