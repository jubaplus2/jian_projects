...
def ncreatoutput1():
	...
	lheader, weeklength, prevweekdf, weekinfodict = previousweekFile('/disk2/arnab/rentrak/week8/2276769/Show2276769W8.csv')
	print lheader
	print 'hit ncreatoutput1'
	print ' first previousweekFile written'
	header = '\t'.join(lheader)+'\n'
	f = open('Show2276769W9.csv','w')
        f.write(header)
        f.close()
				...
				if tt.get(elements1[0]):
					...
					for elements in usngzip4list:
						...
						if not searchinDF(prevweekdf,weekinfodict, inputlist, elements1[1]):
							...
							df.to_csv('Show2276769W9.csv','\t', index = False, mode='a', header=False)
	print 'dumping data'
	prevweekdf.to_csv('Show2276769W9.csv','\t', index = False, mode='a', header=False)
	print 'arnab6'
