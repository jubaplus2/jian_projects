...
def Smoothie_King_weekly():
    ...
    monday_this_week_weekday=this_monday.day
    tuesday_this_week_weekday=(this_monday+datetime.timedelta(days=1)).day
    wednesday_this_week_weekday=(this_monday+datetime.timedelta(days=2)).day
    thursday_this_week_weekday=(this_monday+datetime.timedelta(days=3)).day
    friday_this_week_weekday=(this_monday+datetime.timedelta(days=4)).day
    previous_saturday_weekday=(this_monday-datetime.timedelta(days=2)).day
    previous_sunday_weekday=(this_monday-datetime.timedelta(days=1)).day


    min_day_of_month_last_Sat_to_this_Fri=min([monday_this_week_weekday,tuesday_this_week_weekday,wednesday_this_week_weekday,thursday_this_week_weekday,friday_this_week_weekday,previous_saturday_weekday,previous_sunday_weekday])

    # force_run_ind=1
    force_run_ind=0

    if min_day_of_month_last_Sat_to_this_Fri==1 or force_run_ind==1:
...
Jian
        ...
        matched_df_with_SG=pd.read_excel("/home/jian/Projects/Smoothie_King/TA/Celery_SK_TA/QC_Address_With_SG_JL_2019-02-12.xlsx",sheet_name="Matched_Stores",dtype=str)
        ...
        latest_force_list_excel=glob.glob("/home/jian/Projects/Smoothie_King/SK_celery/force_store_list/*.xlsx")
        latest_force_list_excel=sorted(latest_force_list_excel)[-1]
        forced_exclude_store_list=pd.read_excel(latest_force_list_excel,sheet_name="closed",dtype=str)['storenumber'].tolist()
        forced_exclude_store_list=[x for x in forced_exclude_store_list if x!="nan"]
        store_list_4_weeks=store_list_4_weeks[~store_list_4_weeks['storenumber'].isin(forced_exclude_store_list)]

        '''
        # Force new opening is going to aggregated into the /home/jian/Projects/Smoothie_King/adjust_for_pending_stores/

        
        force_added_stores=pd.read_excel(latest_force_list_excel,sheet_name="opening",dtype=str)
        force_added_stores=force_added_stores[~force_added_stores['storenumber'].isin(store_list_4_weeks['StoreNumber'].tolist())]
        store_list_4_weeks=store_list_4_weeks.append(force_added_stores)
        '''

        store_list_4_weeks=store_list_4_weeks.reset_index()
        del store_list_4_weeks['index']
        del sales_data_4_weeks_agg_count
	    
        store_GPS_verified=pd.read_excel("/home/jian/Projects/Smoothie_King/TA/QC_address/SK_store_listed_verified_JL_2019-12-03.xlsx",sheet_name="assigned_lat_long",dtype=str)
	        ...
	        if len(group)>1:
	            ...
	            for i in range(len(group)):
	                ...
	                for j in range(i+1,len(group)):
	                    ...
	                    try:
	                        dist=haversine(zip_hold_center,zip_var_center,unit="mi")
	                    except:
	                        dist=np.nan
	        ...
	        for i in range(len(summary_by_TA_to_revise)):
	            ...
	            while len(store_list)>=2:
	                ...
	                for store_var in store_list:
	                    ...
	                    dist=haversine(store_hold_center,store_var_center,unit="mi")
	            ...
	            for store in store_list:
	                ...
	                dist_a=haversine(store_a_center,store_center,unit="mi")
	                dist_b=haversine(store_b_center,store_center,unit="mi")
	                if dist_a<dist_b:
	                    sub_group="a"
	                else:
	                    sub_group="b"
    ...
    # In[73]:
        ...
        rfa_mapping['DMA']=rfa_mapping['DMA'].fillna("nan")
