...
def test_get_conda_version(qtbot, api):
    def worker_ready(worker, output, error):
        assert (output.startswith('4.3.') or output.startswith('4.4.'))
...
#    worker_r = api.remove_environment(name=name)
...
#    worker_r = api.remove_environment(prefix=prefix)
#    with qtbot.waitSignal(worker_r.sig_finished, timeout=DEFAULT_TIMEOUT):
#        pass
...
def test_install(qtbot, api):
    pass
    # name=None,prefix=None,pkgs=None,dep=True,channels=None,token=None
