...
class EtlPos:
    ...
    def preprocess(self):
        ...
        def choose_id_from_crm():
            ...
            # jian 0810
            sql_date_pos_start="'"+str(self.pos_start_date_id_filter)+"'"
            clause_join = "inner join (select distinct customer_id_hashed from %s where transaction_dt >= %s) as t2 on crm.customer_id_hashed=t2.customer_id_hashed"%(self.pos_table,sql_date_pos_start)
            if self.random_size:
            ...
            list_of_clause = [clause_create_table, clause_select, clause_join, clause_where]
    ...
    def build_table_1_with_split(self): # jian 0807, "processing_part_2.ipynb"
        ...
        p = Pool(processors)
        result=pool.starmap(get_dist_output_df, zip(list_of_input_all_us_zip_list, repeat(df_store_list), repeat(zip_centers)))
    ...
    def output_to_csv(self):
        ...
        output_table_2_1()
        output_table_2_2()
    def dump_table_name(self):
        dict_table_names={}
        path_json_table_names="./table_names_%s.json"%str(self.pos_end_date).replace("-","")

        dict_table_names.update({"table_dcm_id":self.table_dcm_id})
        dict_table_names.update({"table_id_from_crm":self.table_id_from_crm})
        dict_table_names.update({"table_filtered_id":self.table_filtered_id})
        dict_table_names.update({"table_filtered_pos":self.table_filtered_pos})
        dict_table_names.update({"table_filtered_crm":self.table_filtered_crm})
        dict_table_names.update({"table_max_trans_order":self.table_max_trans_order})
        dict_table_names.update({"table_2_1":self.table_2_1})
        dict_table_names.update({"table_2_2":self.table_2_2})
